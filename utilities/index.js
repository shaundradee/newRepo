...
* --(Similar to buildClassificationGrid, but each
*   has an "unfavorite" button/form so user can 
...
Util.buildFavoritesGrid = async function (favorites) {
  ...
  if (favorites.length > 0) {
    ...
    favorites.forEach(vehicle => {
      ...
      grid += '<span>$'
        + new Intl.NumberFormat('en-US').format(vehicle.inv_price) + '</span>';
      grid += '<form action="/favorites/remove" method="POST" onsubmit="return validateFavoriteForm(this);">';
      grid += '<input type="hidden" name="inv_id" value="' + vehicle.inv_id + '">';
      grid += '<input type="hidden" name="source" value="favorites">';
      grid += '<button type="submit" class="favorite-btn remove">Remove from Favorites</button>';
      grid += '</form>';
      grid += '</div>';
      grid += '</li>';
    });
    ...
    grid = '<p class="notice">You have not added any vehicles to your favorites yet.</p>';
  }
  return grid;
}
